...
router.post('/', (req, res) => {
    ...
    if (!user_id || !table_number || !number_of_guests || !date_time) {
        return res.status(400).json({ error: 'User ID, table number, number of guests, and date-time are required.' });
    }
    ...
    db.get(checkSql, [user_id, date_time], (err, row) => {
        ...
        if (row) {
            return res.status(409).json({ error: 'Reservation for this user at this date and time already exists.' });
        }

        let insertSql = `INSERT INTO Reservations (user_id, table_number, number_of_guests, date_time) VALUES (?, ?, ?, ?)`;
        db.run(insertSql, [user_id, table_number, number_of_guests, date_time], function(err) {
            if (err) {
                return res.status(400).json({ error: err.message });
            }
